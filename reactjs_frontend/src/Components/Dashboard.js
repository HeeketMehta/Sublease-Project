...
function DashboardComponent() {
    ...
    useEffect(()=>{
        ...
        async function fetchMyAPI() {
            console.log("CALLING THE API FROM DASHBOARD.JS");
            const UserData = {username: localStorage_data_username}
            const res = await axios.post("/api/get_listings_of_user", UserData).then((response) => {
                ...
                // Object.values(response_data_listing).map(data => {
                //     Object.entries(data).map(([k,v],v1)=> {
                //     console.log("For key - " + k + " we have value = " + v);
                //     })
                // })
                
                console.log(response_data_listing);
                setResult(response_data_message);
                // setListingData(response_data_listing);
            }).catch((err)=>{
